...
class black_box_benchmarks(object):
    ...
    def __init__(self, shadow_train_performance, shadow_test_performance, 
        ...
        self.s_tr_conf = np.take_along_axis(self.s_tr_outputs, self.s_tr_labels[:, None], axis = 1)
        self.s_te_conf = np.take_along_axis(self.s_te_outputs, self.s_te_labels[:, None], axis = 1)
        self.t_tr_conf = np.take_along_axis(self.t_tr_outputs, self.t_tr_labels[:, None], axis = 1)
        self.t_te_conf = np.take_along_axis(self.t_te_outputs, self.t_te_labels[:, None], axis = 1)
    ...
    def _mem_inf_thre(self, v_name, s_tr_values, s_te_values, t_tr_values, t_te_values):
        ...
        mem_inf_acc = 0.5*(t_tr_acc + t_te_acc)
        print('For membership inference attack via {n}, the attack acc is {acc1:.3f}, with train acc {acc2:.3f} and test acc {acc3:.3f}'.format(n=v_name, acc1=mem_inf_acc, acc2=t_tr_acc, acc3=t_te_acc))
        return t_tr_acc, t_te_acc
    ...
    def _mem_inf_benchmarks(self, all_methods=True, benchmark_methods=[]):
        ...
        if (all_methods) or ('confidence' in benchmark_methods):
            ret['confidence'] = self._mem_inf_thre('confidence', self.s_tr_conf, self.s_te_conf, self.t_tr_conf, self.t_te_conf)
        if (all_methods) or ('entropy' in benchmark_methods):
            ret['entropy'] = self._mem_inf_thre('entropy', -self.s_tr_entr, -self.s_te_entr, -self.t_tr_entr, -self.t_te_entr)
        if (all_methods) or ('modified entropy' in benchmark_methods):
            ret['m_entropy'] = self._mem_inf_thre('modified entropy', -self.s_tr_m_entr, -self.s_te_m_entr, -self.t_tr_m_entr, -self.t_te_m_entr)
